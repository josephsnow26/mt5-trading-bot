...
adaptive per-symbol deadline applied — no silently skipped days):
  ...
  USTECm @ 15:00 UTC : NASDAQ-100 index CFD, added this revision.
    ...
    (60.4%->52.9%) — 15:00 chosen for consistency. Sizing at the 0.01 lot
    floor is well-behaved here (~$0.80 risk on a $90 balance against a 1%
    / $0.90 target) — unlike gold/BTC/silver, no MIN_BALANCE gate needed.
...
CHANGE LOG (this revision):
  - Added USTECm to SYMBOL_CONFIG (trigger_hour=15, offset=12, sl=40,
    trail=20, be_trigger=40 — all in raw index points), WEEKEND_CLOSED_
    SYMBOLS (USTECm closes for the weekend the same as FX/gold/silver —
    confirmed from the CSV's own weekly gap pattern, Fri ~20:45-21:45 UTC
    close to Sun ~22:00-23:00 UTC reopen), and _pip_value_per_lot() ($1.00
    per lot per 1.00-point move, confirmed directly from
    mt5.symbol_info("USTECm") — contract_size=1.0, tick_value=0.01,
    tick_size=0.01 — not inferred the way BTC/XAG's values still are).
    Added USTEC_ENABLED / USTEC_MIN_BALANCE (unused — sizing already
    behaves at the floor, see backtest summary above) to __init__'s
    traded_symbols gating, matching the existing GOLD/BTC/SILVER pattern.
...
# docstring's USTECm note. Sizing at the min-lot floor is already
# well-behaved (~$0.80 risk against a $0.90 / 1% target on a $90 balance),
# so unlike GOLD/BTC/SILVER_MIN_BALANCE above, USTEC_MIN_BALANCE isn't a
# real gate — kept only for symmetry with the other symbols' pattern and
# documentation of the math it was checked against.
USTEC_ENABLED = True
USTEC_MIN_BALANCE = 0.0  # no floor-risk problem at any tested balance level
...
def _pip_value_per_lot(symbol: str, price: float) -> float:
    ...
    $1/point for USTECm, which IS confirmed directly from
    mt5.symbol_info("USTECm") (contract_size=1.0, tick_value=0.01,
    tick_size=0.01 -> $1.00 per 1.00-point move per lot). See the module
    docstring's notes before trusting position sizes on BTC or XAG."""
    if symbol == "USDJPYm":
        return 1000.0 / price
    if symbol == "XAUUSDm":
        return 100.0
