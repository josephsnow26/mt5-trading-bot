...
class RestrictiveVolumeStrategy:
    ...
    def __init__(
        ...
        self.blocked_dates: list[str] = []
        self.min_bars = max(ema_period, macd_slow + macd_signal, volume_ma_period, atr_period) + 5
    ...
    def _check_volume_spike(self, volume: pd.Series, volume_ma: pd.Series) -> tuple[bool, str]:
    ...
    def generate_signal(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if not trend:
                return {**self._empty_signal("No clear trend"), "filters_passed": filters}
            ...
            if not cross:
                return {**self._empty_signal("No MACD cross"), "filters_passed": filters}
            ...
            if not vol_ok:
                return {**self._empty_signal(vol_msg), "filters_passed": filters}
            ...
            if not dist_ok:
                return {**self._empty_signal(dist_msg), "filters_passed": filters}
            ...
            if not mom_ok:
                return {**self._empty_signal(mom_msg), "filters_passed": filters}
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            "favorite_weekdays": [["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][d] for d in self.favorite_weekdays],
